<script>
    $(document).ready(function() {
        $('#formValidation').find('[name="dob"]').val(
                                        ...
                                        '{!! $item->monday_finish !!}' === '' &&
                                        '{!! $item->tuesday_finish !!}' === '' &&
                                        '{!! $item->wednesday_finish !!}' === '' &&
                                        '{!! $item->thursday_finish !!}' === '' &&
                                        '{!! $item->friday_finish !!}' === '' &&
                                        '{!! $item->saturday_finish !!}' === '' &&
                                        '{!! $item->sunday_finish !!}' === '' ?
                                        '' : [
                                                '{!! $item->monday_init !!}',
                                                '{!! $item->tuesday_init !!}',
                ...
                searchmap: {
                    validators: {
                        callback: {
                            ...
                            callback: function(value, validator, $field) {
                                var ubication = $('input[name=ubication]:checked').val();
                                if(ubication == 1 && (value != '' && value != null)){
                                    return true;
                                }else if(ubication == 0){
                                    return true;
                                }else{
        ...
        // Set content from item
        CKEDITOR.instances.description_content.setData('{!! $item->description !!}');
    ...
    // Ubication
    $('input[name=ubication]').change(function(){
        $('#formValidation').formValidation('resetField', 'searchmap');
        enableDisableMap();
    });

    function enableDisableMap(){
        var value = $('input[name=ubication]:checked').val();
        if(value == 0){
            $('#searchmap').val('');
            $('#lat').val('');
            $('#lng').val('');
